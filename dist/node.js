...
// ------------------------------
function normalizeContractIdForUrl(contractId) {
    return strip0x(String(contractId || "").trim());
}
function shouldRetryWithFeeder(text) {
    const t = String(text || "");
...
function parseRuesFrame(data) {
    // RUES frame format:
    // [u32 LE headers_len][headers_json_bytes][body_bytes]
    const view = new DataView(data);
    if (view.byteLength < 4)
        throw new Error("Invalid RUES frame");
    const headersLen = view.getUint32(0, true);
    if (4 + headersLen > view.byteLength)
        throw new Error("Invalid RUES headers length");
...
async function subscribeRues(f, url, sessionId, signal) {
...
export function createDuskNodeClient(opts) {
    ...
    const contractCall = async (contractId, fnName, body, callOpts = {}) => {
    ...
    const waitForTxExecuted = async (hash, waitOpts = {}) => {
        const base = getBaseUrl();
        if (!base)
            throw new Error("DuskNodeClient: baseUrl is empty");
