...
export function isRpcErrorLike(err) {
    return (typeof err === "object" &&
        err !== null &&
        "message" in err &&
        typeof err.message === "string" &&
        ("code" in err ? typeof err.code === "number" : true));
}
/**
 ...
 */
export function normalizeError(err, fallbackMessage = "Unknown error") {
    ...
    if (typeof err === "string") {
        return new DuskSdkError(err);
    }
    try {
        return new DuskSdkError(JSON.stringify(err));
    }
    catch {
        return new DuskSdkError(fallbackMessage);
    }
}
/**
