...
export class MochaviConnectButtonElement extends HTMLElement {
    ...
    _ensureWalletAndModal() {
        ...
        if (!this._modal && this._wallet) {
            ...
            const modalOpts = {};
            if (appName)
                modalOpts.appName = appName;
            if (installUrl)
                modalOpts.installUrl = installUrl;
    ...
    _update(st) {
        ...
        try {
            this.dispatchEvent(new CustomEvent("dusk-state", { detail: st, bubbles: true, composed: true }));
        }
        catch {
            // ignore
        }
        ...
        const status = walletStatus(st);
        if (this._btn)
            this._btn.setAttribute("data-status", status);
        if (!this._label || !this._avatar || !this._net)
            return;
...
export function defineMochaviConnectButton(tagName = "mochavi-connect-button") {
 ...
 */
export function createMochaviConnectButton(options = {}) {
    defineMochaviConnectButton();
    const el = document.createElement("mochavi-connect-button");
    if (options.appName)
        el.setAttribute("app-name", options.appName);
    if (options.installUrl)
        el.setAttribute("install-url", options.installUrl);
    if (options.closeOnConnect !== undefined)
        el.setAttribute("close-on-connect", String(options.closeOnConnect));
    if (options.hideNetwork)
        el.setAttribute("hide-network", "");
    if (options.connectText)
        el.setAttribute("connect-text", options.connectText);
    if (options.lockedText)
        el.setAttribute("locked-text", options.lockedText);
    if (options.installText)
        el.setAttribute("install-text", options.installText);
    if (options.walletOptions)
        el.walletOptions = options.walletOptions;
    if (options.wallet)
        el.wallet = options.wallet;
    if (options.modal)
        el.modal = options.modal;
    return el;
}
